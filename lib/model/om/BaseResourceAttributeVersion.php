...
/**
 * Base class that represents a row from the 'resource_attribute_version' table.
 *
 * 
 *
...
abstract class BaseResourceAttributeVersion extends BaseObject  implements Persistent {
  ...
  const PEER = 'ResourceAttributeVersionPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        ResourceAttributeVersionPeer
	 */
	...
	/**
	 * Initializes internal state of BaseResourceAttributeVersion object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     ResourceAttributeVersion The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->attribute_name = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->attribute_value = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->resetModified();
			...
			return $startcol + 3; // 3 = ResourceAttributeVersionPeer::NUM_COLUMNS - ResourceAttributeVersionPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = ResourceAttributeVersionPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseResourceAttributeVersion:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
			...
			ResourceAttributeVersionPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[2], $arr)) $this->setAttributeValue($arr[$keys[2]]);
	}
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of ResourceAttributeVersion (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     ResourceAttributeVersion Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     ResourceAttributeVersionPeer
	 */
	 ...
	 * Otherwise if this ResourceAttributeVersion has previously been saved, it will retrieve
	 * related ResourceAttributeVersionHashs from storage. If this ResourceAttributeVersion is new, it will return
		...
		if ($this->collResourceAttributeVersionHashs === null) {
			if ($this->isNew()) {
			   $this->collResourceAttributeVersionHashs = array();
				...
				$criteria->add(ResourceAttributeVersionHashPeer::RESOURCE_ATTRIBUTE_VERSION_ID, $this->id);
	...
	public function addResourceAttributeVersionHash(ResourceAttributeVersionHash $l)
	{
		...
		if (!in_array($l, $this->collResourceAttributeVersionHashs, true)) { // only add it if the **same** object is not already associated
			...
			$l->setResourceAttributeVersion($this);
		}
	}
	 ...
	 * Otherwise if this ResourceAttributeVersion is new, it will return
	 * an empty collection; or if this ResourceAttributeVersion has previously
	 * been saved, it will retrieve related ResourceAttributeVersionHashs from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in ResourceAttributeVersion.
	 */
	public function getResourceAttributeVersionHashsJoinResourceVersion($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(ResourceAttributeVersionPeer::DATABASE_NAME);
		}
		...
		if ($this->collResourceAttributeVersionHashs === null) {
			if ($this->isNew()) {
				...
				$this->collResourceAttributeVersionHashs = ResourceAttributeVersionHashPeer::doSelectJoinResourceVersion($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
